                  ...
                  <template v-for="(schedule, index) in schedules" :key="schedule?.id">
                    <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                      ...
                      <td class="flex items-center px-6 py-4 space-x-3">
                        <button @click="openEditModal(schedule.id)" class="text-emerald-600 hover:underline">Add task</button>
                        <router-link :to="{ name: 'ScheduleView', params: { id: schedule.id } }" class="text-blue-600 hover:underline">View</router-link>
                        <button @click="openEditModal(schedule.id)" class="text-blue-600 hover:underline">Edit</button>
                        <button @click="onRemove(schedule?.id)" class="text-red-600 hover:underline">Remove</button>
                      </td>
                    </tr>
                    ...
                    <tr v-if="expandedScheduleId === schedule?.id">
                      <td colspan="9">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border mt-2">
                          <thead class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                              <th scope="col" class="px-6 py-3"></th>
                              <th scope="col" class="px-6 py-3">Title</th>
                          ...
                          <tbody>
                            <template v-for="(task, taskIndex) in schedule?.project?.task" :key="task?.id">
                              <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                              <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <button
                              ...
                              <td class="px-6 py-4">{{ task?.assigned_to?.first_name }}</td>
                              ...
                              <td class="flex items-center px-6 py-4 space-x-3">
                                <button @click="openEditModal(task.id)" class="text-emerald-600 hover:underline">Add activity</button>
                                <router-link :to="{ name: 'TaskView', params: { id: task.id } }" class="text-blue-600 hover:underline">View</router-link>
                                <button @click="openEditModal(task.id)" class="text-blue-600 hover:underline">Edit</button>
                                <button @click="onRemove(task.id)" class="text-red-600 hover:underline">Remove</button>
                              </td>
                            </tr>
                            ...
                            <tr v-if="expandedTaskId === task.id">
                              <td colspan="11">
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border mt-2">
                                  ...
                                  <tbody>
                                    <tr v-for="(activity, taskIndex) in task?.activity" :key="activity?.id" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                      <!-- {{activity}} -->
                                      <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ task.name }}</td>
                                      <td class="px-6 py-4">{{ activity?.name }}</td>
                                      <td class="px-6 py-4">{{ activity?.unit?.name }}</td>
                                      <td class="px-6 py-4">{{ activity?.quantity }}</td>
                                      <td class="px-6 py-4">{{ activity?.start_date }}</td>
                                      <td class="px-6 py-4">{{ activity?.end_date }}</td>
                                      <td class="px-6 py-4">{{ activity?.duration }}</td>
                                      <td class="px-6 py-4">{{ activity?.priority?.name }}</td>
                                      <td class="px-6 py-4">{{ activity?.progress }}</td>
                                      <td class="px-6 py-4">{{ activity?.approval }}</td>
                                      <td class="px-6 py-4">{{ activity?.status?.name }}</td>
                                      <td class="px-6 py-4">{{ activity.created_at }}</td>
                                      <td class="flex items-center px-6 py-4 space-x-3">
                                        <button  class="text-emerald-600 hover:underline">Request</button>
                                        <router-link :to="{ name: 'ActivityView', params: { id: task.id } }" class="text-blue-600 hover:underline">View</router-link>
                                        <button @click="openEditModal(task.id)" class="text-blue-600 hover:underline">Edit</button>
                                        <button @click="onRemove(task.id)" class="text-red-600 hover:underline">Remove</button>
                                      </td>
                                    </tr>
...
const toggleSchedule = (scheduleId) => {
  expandedScheduleId.value = expandedScheduleId.value === scheduleId ? null : scheduleId;
