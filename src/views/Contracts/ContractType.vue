<template>
    <PageComponentVue>
        ...
        <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div v-else>
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(type, index) in types" :key="index" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
        ...
        <fwb-modal v-if="isShowModal" @close="closeModal">
            ...
            {{ isEditing ? 'Edit Contract Type' : 'Add Contract Type' }}
            </div>
            </template>
            <template #body>
            <form @submit="createContractType">
...
const types = computed(() => store.contractTypes)
...
function createContractType(e) {
    e.preventDefault()
    const action = isEditing.value ? 'updated' : 'created'
    const apiCall = isEditing.value ? store.updateContractType : store.createContractType
    apiCall({...type.value})
        .then(() => {
            store.notify({
                type: 'success',
                message: 'The contract type was successfully ' + action
            })
            closeModal()
