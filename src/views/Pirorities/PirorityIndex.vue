<template>
    <PageComponentVue>
        ...
        <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div v-else>
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(pirority, index) in pirorities" :key="index" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <td class="flex items-center px-6 py-4 space-x-3">
                                <button v-if="pirority.id"  @click="openEditModal(pirority.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                                <button @click="onRemove(pirority?.id)" class="font-medium text-red-600 dark:text-red-500 hover:underline">Remove</button>
        ...
        <fwb-modal v-if="isShowModal" @close="closeModal">
            ...
            {{ isEditing ? 'Edit Priroty' : 'Add Priroty' }}
            </div>
            </template>
            <template #body>
            <form @submit="createPirority">
                    ...
                    <div v-if="errMsg != false" id="alert-border-2" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 dark:text-red-400 dark:bg-gray-800 dark:border-red-800" role="alert">
                        ...
                        <div class="grid gap-4 mb-4 sm:grid-cols-1">
                            <div>
                                ...
                                <input type="text" v-model="pirority.name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="priroty name">
                            ...
                            <div>
                                ...
                                <input type="text" v-model="pirority.slug" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="priroty name">
                                <!-- <div v-if="validationErr.name" class="text-red-500 text-xs mt-1">
                                {{ validationErr.name[0] }}
                                </div> -->
                            </div>
                            <div>
                                ...
                                <input type="text" v-model="pirority.color" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="priroty name">
                                <!-- <div v-if="validationErr.name" class="text-red-500 text-xs mt-1">
                                {{ validationErr.name[0] }}
                                </div> -->
                            </div>
...
const store = usePirorityStore()
const isLoading = computed(() => store.isLoading)
const notification = computed(() => store.notification)
const pirorities = computed(() => store.pirorities)
...
function openEditModal(selectedPirority) {
    if (selectedPirority) {
        store.getPirority(selectedPirority)
    }
    isShowModal.value = true
...
function createPirority(e) {
    e.preventDefault()
    const action = isEditing.value ? 'updated' : 'created'
    const apiCall = isEditing.value ? store.updatePirority : store.createPirority
    apiCall({...pirority.value})
        .then(() => {
            store.notify({
                type: 'success',
                message: 'The pirority was successfully ' + action
            })
            closeModal()
            store.getPirorities()
        })
