        ...
        <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div v-else>
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(type, index) in equipmentTypes" :key="index" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <td class="flex items-center px-6 py-4 space-x-3">
                                <button v-if="type.id"  @click="openEditModal(type.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                                <button @click="onRemove(type?.id)" class="font-medium text-red-600 dark:text-red-500 hover:underline">Remove</button>
                    ...
                    <div v-if="errMsg != false" id="alert-border-2" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 dark:text-red-400 dark:bg-gray-800 dark:border-red-800" role="alert">
                        ...
                        <div class="grid gap-4 mb-4 sm:grid-cols-1">
                            <div>
                                ...
                                <input type="text" v-model="type.name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Equipment type name">
                            ...
                            <div>
                                ...
                                <textarea id="message" rows="4" v-model="type.description" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Leave a comment..."></textarea>
                            </div>
                        </div>
...
const equipmentTypes = computed(() => store.equipmentTypes)
...
watch(() => store.currentEquipmentType,
            (newVal, oldVal) => {
                if (newVal !== undefined) {
                    type.value = {
...
function openEditModal(selectedType) {
    if (selectedType) {
        store.getEquipmentType(selectedType)
    }
    isShowModal.value = true
...
function createType(e) {
    ...
    const apiCall = isEditing.value ? store.updateEquipmentType : store.createEquipmentType
    apiCall({...type.value})
        .then(() => {
            store.notify({
                type: 'success',
                message: 'The equipment type was successfully ' + action
            })
            closeModal()
