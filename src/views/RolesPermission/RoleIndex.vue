    ...
    <PageComponentVue>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
               <div class="relative w-full mb-6 group">
                <router-link :to="{name: 'Permissions'}" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-white">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5V6.75a4.5 4.5 0 1 1 9 0v3.75M3.75 21.75h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H3.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                    </svg>
                    <p class="t-3 font-bold text-white dark:text-white">Permissions</p>
                </router-link>
              </div>
         </div>
        <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div v-else>
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(role, index) in roles" :key="index" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <td class="flex items-center px-6 py-4 space-x-3">
                                <button v-if="role.id"  @click="openEditModal(role.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                                <button @click="onRemove(role?.id)" class="font-medium text-red-600 dark:text-red-500 hover:underline">Remove</button>
...
//   store.searchGrade({...search.value})
...
function saveRole(e) {
    ...
    const apiCall = isEditing.value ? store.updateRoles : store.createRole
    apiCall({...role.value})
        .then(() => {
            store.notify({
                type: 'success',
