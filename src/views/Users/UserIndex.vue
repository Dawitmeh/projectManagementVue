    ...
    <PageComponentVue>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
             <div class="relative w-full mb-6 group">
                <router-link :to="{name: 'Roles'}" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-white">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                    </svg>

                    <p class="t-3 font-bold text-white dark:text-white">Roles & Permissions</p>
                </router-link>
              </div>
         </div>
        <div class="mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div v-else>
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(user, index) in users" :key="index" class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <td class="flex items-center px-6 py-4 space-x-3">
                                <button v-if="user.id"  @click="openEditModal(user.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                                <button @click="onRemove(user?.id)" class="font-medium text-red-600 dark:text-red-500 hover:underline">Remove</button>
        ...
        <fwb-modal v-if="isShowModal" @close="closeModal">
            ...
            {{ isEditing ? 'Edit Users' : 'Add Users' }}
            </div>
            </template>
            <template #body>
                        ...
                        <div v-if="errMsg != false" id="alert-border-2" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 dark:text-red-400 dark:bg-gray-800 dark:border-red-800" role="alert">
                            ...
                            <div class="grid gap-6 mb-6 md:grid-cols-2 ">
                                ...
                                <div>
                                    ...
                                    <select id="countries" v-model="user.role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
...
const users = computed(() => store.staffs)
const roles = computed(() => storeRole.roles)
...
function onImageChoose(event) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    reader.onload = (event) => {
      ...
      user.value.profile_picture = fileArray;
      user.value.profile_picture_url = [reader.result];
...
function createUser(e) {
    e.preventDefault()
    if (validateForm()) {
        store.createUsers({ ...user.value })
            .then(() => {
                store.notify({
                    type: 'success',
                    message: 'The user was successfully ' + action
                })
                isShowModal.value = false
                store.getUsers()
                store.getRoles()
